...
- Q&A: where an LLM can be asked deeper question about the survey data and will create charts summarising answers
...
- Data preprocessing has two main outcomes:
	...
	- Also reformats 'checkbox' style questions into one-hot-encoded columns for each possible answer
	- Saves output as .parquet for faster operation with DuckDB backend
- Survey app:
	- Streamlit (python) app with a DuckDB backend for querying the data
	- DuckDB will easily work with datasets up to 1m rows for preprocessing data
	- Aggregated data is then handled within the app by pandas and plotly for visualisations
...
## Setup (NOTE: tested with Python 3.11.14)
- Create and activate a python virtual environment in the directory where the files are contained
